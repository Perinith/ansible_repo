...
class PowerMaxVolume(object):
    ...
    def get_volume(self):
        ...
        if not volume_list or len(volume_list) == 0:
            LOG.debug(
                'No volume found with volume identifier {0} in'
                'storage group {1}'.format(volume_name, sg_name))
    ...
    def get_volume_by_nativeid(self, native_id):
        try:
            ...
            LOG.error('Got error {0} while getting details of '
                      'volume {0}'.format(str(e), native_id))
            return None
    ...
    def delete_volume_deallocate(self, vol_id):
        ...
        try:
            ...
            self.module.fail_json(
                msg=error_msg.format(vol_id, str(e)))
        ...
        try:
            ...
            if 'A free of all allocations is required ' in str(e):
                ...
                error_msg = 'Delete volume {0} failed with error ' \
                            '{1} '
                LOG.error(error_msg.format(vol_id, str(e)))
                self.module.fail_json(msg=error_msg.format(vol_id,
                                                           str(e)))
    ...
    def rename_volume(self, vol_id, vol_new_name):
        ...
        try:
            ...
            LOG.error('Rename volume {0} failed with error '
                      '{1}'.format(vol_id, str(e)))
            self.module.fail_json(msg='Rename volume {0} failed.'
                                  .format(vol_id))
    ...
    def expand_volume(self, vol_id, size_in_gb, existing_vol_size):
        ...
        try:
            if size_in_gb < existing_vol_size:
                ...
                LOG.info(
                    'Expanding volume capacity from {0} GB to {1}'
                    'GB.'.format(existing_vol_size, size_in_gb))
            ...
            LOG.error('Expand volume {0} failed with error {1}'.
                      format(vol_id, str(e)))
            self.module.fail_json(
                msg='Expand volume {0} failed with '
                    'error '.format(vol_id, str(e)))
    ...
    def create_volume(self, vol_name, sg_name, size, cap_unit):
        ...
        try:
            ...
            self.module.fail_json(
                msg='Create volume {0} failed with '
                    'error '.format(vol_name, str(e)))
    ...
    def move_volume_between_storage_groups(self, vol_id, vol_name,
        ...
        try:
            ...
            error_message = 'Move volume {0} from SG {1} to SG {2}'\
                            ' failed with error {3}'
            LOG.error(error_message.format(vol_name, sg_name,
                                           new_sg_name, str(e)))
    ...
    def perform_module_operation(self):
            ...
            if size is not None and size > 0:
                size = utils.get_size_in_gb(size,
                                            self.module.params[
                                                'cap_unit'])
                LOG.info(
                    'Existing Size: {0} GB, Specified Size: {1}'
                    ' GB'.format(existing_vol_size, size))
            changed = self.expand_volume(vol_id, size,
                                         existing_vol_size)
        ...
        if state == 'present' and vol and new_name:
            ...
            if new_name != vol_name:
                ...
                changed = self.rename_volume(vol_id, new_name) \
                          or changed
        ...
        """Finally update the module changed state and saving 
