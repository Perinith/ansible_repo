...
class PowerMaxHostGroup(object):
    ...
    def get_hostgroup(self, hostgroup_name):
        ...
        try:
            LOG.info('Getting host group {0} details'
            ...
            LOG.error('Got error {0} while getting details of '
                      'host group {0}'
    ...
    def create_hostgroup(self, hostgroup_name):
        ...
        try:
            if emptyHostGroupFlag:
                LOG.info('Creating empty HostGroup {0} with parameters '
                         '{1}'.format(hostgroup_name, param_list))
                ...
                for host in hosts:
                    try:
                        ...
                        LOG.error('The host {0} is not found on array'
                                  .format(host))
        ...
        if hosts \
           and (set(hosts).issubset(set(existing_hosts))) :
                LOG.info('Hosts are already present in host group {0}'
                         .format(existing_hosts))
                return False

        add_list = self._get_add_hosts(existing_hosts, hosts)
        if len(add_list) > 0:
            try:
                LOG.info('Adding hosts {0} to host group {1}'
                        .format(add_list, hostgroup_name))
            ...
            LOG.info('No hosts to add to host group {0}'
                    .format(hostgroup_name))
            return False
    ...
    def remove_hosts_from_hostgroup(self, hostgroup_name, hosts):
        ...
        if len(rem_list) > 0:
            try:
                LOG.info('Removing hosts {0} from host group {1}'
                        .format(rem_list, hostgroup_name))
                self.provisioning.modify_hostgroup(hostgroup_name,
                                                   remove_host_list
                                                   =rem_list)
                return True
            ...
            LOG.info('No hosts to remove from host group {0}'
                    .format(hostgroup_name))
            return False
    ...
    def modify_host_flags(self, hostgroup_name, received_host_flags):
        ...
        if new_flags_dict == current_flags:
            ...
            try:
                LOG.info('Modifying host group flags for host {0} with'
                         '{1}'.format(hostgroup_name, new_flags_dict))
                self.provisioning.modify_hostgroup(hostgroup_name,
                                                   new_flags_dict)
                return True
    ...
    def perform_module_operation(self):
        ...
        if (state == 'present' and host_state == 'present-in-group'
            and hostgroup and hosts and len(hosts) > 0):
                LOG.info('Add hosts to host group {0}'
                         .format(hostgroup_name))
        ...
        if (state == 'present' and host_state == 'absent-in-group'
            and hostgroup and hosts and len(hosts) > 0):
                LOG.info('Remove hosts from host group {0}'
                         .format(hostgroup_name))
        ...
        if (state == 'present' and hostgroup and host_flags):
            LOG.info('Modifying host group flags of hostgroup {0} '
                     'to {1}'.format(hostgroup_name, host_flags))
        ...
        if (state == 'present' and hostgroup and new_name):
            if hostgroup['hostGroupId'] != new_name:
                LOG.info('Renaming host group {0} to {1}'
                    .format(hostgroup_name, new_name))
