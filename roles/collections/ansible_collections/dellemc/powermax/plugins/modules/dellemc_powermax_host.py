...
class PowerMaxHost(object):
    ...
    def create_host(self, host_name):
        ...
        try:
            msg = ('Creating host {0} with parameters:initiators={1},'
                   'host_flags={2}')
            LOG.info(msg.format(host_name, initiators,
                                new_host_flags_dict))
    ...
    def remove_host_initiators(self, host_name, initiators):
        ...
        if len(remove_list) > 0:
            try:
                LOG.info('Removing initiators {0} from host {1}'
                        .format(remove_list, host_name))
                self.provisioning.modify_host(host_name,
                                              remove_init_list
                                              =remove_list)
                return True
            ...
            LOG.info('No initiators to remove from host {0}'
                    .format(host_name))
            return False
    ...
    def perform_module_operation(self):
        ...
        if (state == 'present' and host
            ...
            and initiators and len(initiators) > 0):
                LOG.info('Adding initiators to host {0}'
                         .format(host_name))
        ...
        if (state == 'present' and host
            ...
            and initiators and len(initiators) > 0):
                LOG.info('Remove initiators from host {0}'
                         .format(host_name))
        ...
        if state == 'present' and host and host_flags:
            LOG.info('Modifying host flags of host {0} to {1}'
                    .format(host_name, host_flags))
            changed = self.modify_host_flags(host_name, host_flags)\
                      or changed

        if state == 'present' and host and new_name:
            if host['hostId'] != new_name:
                LOG.info('Renaming host {0} to {1}'
                         .format(host_name, new_name))
