...
class PowerMaxPortGroup(object):
    ...
    def get_portgroup(self, portgroup_name):
        ...
        try:
            LOG.info('Getting port group {0} details'
                     .format(portgroup_name))
            return self.provisioning.get_portgroup(portgroup_name)
        except Exception as e:
            LOG.error('Got error {0} while getting details of port '
                      'group {0}'.format(str(e), portgroup_name))
            return None
    ...
    def create_portgroup(self, portgroup_name):
        ...
        try:
            ...
            LOG.info('Creating port group {0} with ports {1}'
                    .format(portgroup_name, ports))
    ...
    def add_ports_to_portgroup(self, portgroup_name):
        ...
        if add_ports is None or len(add_ports) == 0:
            LOG.info(' No Ports to be added to port group  {0}'
                    .format(portgroup))
        ...
        try:
            ...
            LOG.error('Add port {0} to port group {1} failed with'
                      ' error {2}'.format(port, portgroup_name, str(e)))
            self.module.fail_json(msg='Add port {0} to port group {0} '
                                      'failed with error {2}.'
    ...
    def remove_ports_from_portgroup(self, portgroup_name):
        ...
        if rem_ports is None or len(rem_ports) == 0:
            LOG.info('No ports to remove from port group {0}'
                     .format(portgroup))
        ...
        try:
            ...
            LOG.error('Remove port {0} from port group {1} failed with'
                      ' error {2}'.format(port, portgroup_name, str(e)))
    ...
    def perform_module_operation(self):
        ...
        if state == 'present' and port_state == 'present-in-group' \
           and portgroup:
                LOG.info('Adding ports to port group {0}'
                         .format(portgroup_name))
             ...
             and portgroup:
                LOG.info('Removing ports from port group {0}'
                         .format(portgroup_name))
                changed = \
