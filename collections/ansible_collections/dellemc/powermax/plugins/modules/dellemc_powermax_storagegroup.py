...
class PowerMaxStorageGroup(object):
    ...
    def get_volume(self):
        ...
        try:
            ...
            LOG.error('Got error {0} while getting details of volume '
                      '{0}'.format(str(e), vol_id))
            return None
    ...
    def get_volumes_storagegroup(self,sg_name):
        ...
        try:
            LOG.info('Getting the list of volumes from storage group '
                     '{0}'.format(sg_name))
            sg_vol_list = self.provisioning\
                .get_vols_from_storagegroup(sg_name)
            LOG.info('{0} volumes present in storage group {1}'
                     .format(len(sg_vol_list), sg_name))
            return sg_vol_list
        except Exception as e:
    ...
    def get_storage_group(self, sg_name):
        ...
        try:
            LOG.info('Getting storage group {0} details'
                     .format(sg_name))
            return self.provisioning.get_storage_group(sg_name)
        except Exception as e:
            LOG.info('Got error {0} while getting details of storage '
                     'group {1}' .format(str(e), sg_name))
            return None
    ...
    def create_storage_group(self, sg_name):
        ...
        try:
            LOG.info('Creating empty storage group {0} '
                     .format(sg_name))
            resp = self.provisioning\
                        .create_empty_sg(srp, sg_name, slo,
                                         workload=None,
                                         disable_compression
                                         =disable_compression)
            return True, resp
        except Exception as e:
            LOG.error('Failed to create storage group {0} with error '
    ...
    def add_volume_storage_group(self, sg_name):
        ...
        for vol in volumes:
            ...
            if ('vol_id' in vol) and ('vol_name' in vol):
                ...
                continue
            elif ('cap_unit' in vol) or ('vol_name' in vol) \
                    or ('size' in vol):
                if 'cap_unit' in vol:
                    unit = vol['cap_unit']
                else:
                ...
                if 'vol_name' in vol:
                    ...
                    LOG.error('No valid name was specified for the'
                              'volume {0}'.format(vol))
                    self.module.fail_json(msg='No valid name was'
                                              'specified for the volume'
                                              ' {0}'.format(vol))
                if 'size' in vol:
                    size = vol['size']
                else:
                    LOG.error('No valid size was specified for the'
                              'volume {0}'.format(vol))
                ...
                try:
                    ...
                    if not volume_list or len(volume_list) == 0:
                        LOG.info('No volume found with volume '
                                 'identifier {0} in storage group {1}'
                                 .format(name, sg_name))
                        LOG.info('Creating volume with name {0}, size '
                                 '{1} {2}'.format(name, size, unit))
                        self.provisioning \
                    ...
                    LOG.error('Create volume for storage group {0} '
                              'failed with error {1}'
    ...
    def add_existing_volumes_to_sg(self, vol_list, sg_name):
        ...
        if not volumes_to_add:
            LOG.info('The given volumes {0} are already present in '
                     'storage group {1} '.format(vol_ids, sg_name))
        ...
        try:
            ...
            LOG.error('Add existing volume(s) to storage group {0} '
                      'failed with error {1} '.format(sg_name, str(e)))
            self.module.fail_json(msg='Add existing volumes to storage '
                                      'group {0} failed.'
                                  .format(sg_name))
    ...
    def remove_volumes_from_sg(self, vol_list, sg_name):
            ...
            if ('vol_id' in vol) and ('vol_name' in vol):
                ...
                if not volume_list or len(volume_list) == 0:
                    LOG.info(
                        ...
                        'in storage group {1}'.format(vol['vol_name'],
                                                      sg_name))
                elif len(volume_list) > 1:
        ...
        if not volumes_to_remove:
            LOG.info('The given volumes {0} are not present on the '
                     'storage group {1} '.format(vol_ids, sg_name))
            return False, existing_volumes_in_sg
        try:
            ...
            LOG.error('Remove existing volume from storage group {0} '
                      'failed with error {1} '.format(sg_name, str(e)))
    ...
    def modify_sg_srp(self, storage_group, new_srp):
        ...
        try:
            ...
            LOG.error('Modify attribute SRP of storage group {0} failed'
                      ' with error {1}'.format(storage_group, str(e)))
    ...
    def modify_sg_slo(self, sg_name, new_slo):
        ...
        if ('slo' in storage_group and storage_group['slo'].lower()
            ...
            try:
                LOG.info('The existing SG SLO is {0} and the '
                         'desired SG SLO is {1} '
                         .format(storage_group['slo'],
                                 self.module.params[
                                     'service_level']))
                self.provisioning.modify_storage_group(sg_name,
                                                       payload)
            except Exception as e:
                LOG.error('Modify attribute SLO of storage group '
    ...
    def modify_sg_compression(self, sg_name, new_compression):
        ...
        if 'compression' in storage_group \
            ...
            try:
                LOG.info('The existing SG compression is {0} and '
                         'the desired SG compression is {1} '
                         .format(storage_group['compression'],
                                 self.module.params['compression']))
                self.provisioning.modify_storage_group(sg_name,
                                                       payload)
            except Exception as e:
                LOG.error('Modify attribute compression of storage '
                          'group {0} failed with error {1} '
                          .format(sg_name, str(e)))
            ...
            if not self.provisioning \
                ...
                try:
                    ...
                    error_message = 'Adding child SG {0} to parent ' \
                                    ...
                                    'error {2} '
                    LOG.error(error_message.format(child_sg, parent_sg,
                                                   str(e)))
    ...
    def is_storage_group_modified(self, storage_group):
        ...
        LOG.info('Checking if the storage group {0} needs modification.'
                 ' The playbook args are : {1}'
                 .format(storage_group, self.module.params))
        ...
        LOG.info('The storage group {0} needs to be modified and the '
                 'modified param are {1} '
    ...
    def rename_storage_group(self, storage_group, new_sg_name):
        ...
        if storage_group is None:
            ...
            LOG.error(error_message.format(self.module
                                           .params['sg_name'],
                                           new_sg_name))
            self.module.fail_json(msg=error_message
    ...
    def perform_module_operation(self):
            ...
            modified = False
            if storage_group:
                modified, modified_param = self \
                    .is_storage_group_modified(storage_group)
            ...
            if state == 'present' and storage_group and not new_sg_name \
                    and not volumes:
                LOG.info('Get volumes for storage group {0}'
                         .format(sg_name))
            ...
            if state == 'present' and vol_state == 'present-in-group' \
                ...
                LOG.info('Create new volume(s) for storage group '
                         '{0}'.format(sg_name))
                result['add_new_vols_to_sg'], \
                result['storage_group_volumes'] \
                    = self.add_volume_storage_group(sg_name)
                LOG.info('Add existing volume(s) to storage group '
                         '{0}'.format(sg_name))
                result['add_vols_to_sg'], \
                result['storage_group_volumes'] \
                    = self.add_existing_volumes_to_sg(volumes,
                                                      sg_name)
            elif state == 'present' and vol_state == 'absent-in-group' \
                    and storage_group and volumes:
                LOG.info(
                    'Remove existing volume(s) from storage group {0}'
                    .format(sg_name))
            ...
            if state == 'present' and storage_group and modified:
                ...
                result['modify_sg'] \
                    = self.modify_storage_group_attributes(sg_name,
                                                           modified_param)
                result['storage_group_volumes'] \
                    = self.get_volumes_storagegroup(sg_name)
            ...
            if state == 'present' and child_sg_state == 'present-in-group' \
                ...
                LOG.info('Adding child SG to parent storage group {0}'
                         .format(sg_name))
                result['add_child_sg'] \
                ...
                result['storage_group_volumes'] \
                    ...
                    and storage_group and child_sgs:
                LOG.info(
                    'Removing child SG from parent storage group {0}'
                    .format(sg_name))
                result['remove_child_sg'] \
                    = self.remove_child_sg_from_parent_sg(child_sgs
                                                          , sg_name)
                LOG.info('Get volumes for storage group {0}'
                         .format(sg_name))
                result['storage_group_volumes'] \
            ...
            if state == 'present' and new_sg_name:
                LOG.info('Rename SG {0} to new name {1}'
                         .format(sg_name, new_sg_name))
                result['rename_sg'] \
                    = self.rename_storage_group(storage_group,
                                                new_sg_name)
                if result['rename_sg']:
                    sg_name = new_sg_name
                    LOG.info('Get volumes for storage group {0}'
            ...
            if state == 'present' and storage_group:
                LOG.info('Found storage group {0}'
                         .format(storage_group))
